...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 08, 2023 at 01:20 PM
...
--
-- Dumping data for table `accounts_invoice_number_sequence`
--

INSERT INTO `accounts_invoice_number_sequence` (`sequenceId`, `year`, `invoiceNumber`) VALUES
(5, 23, 1),
(6, 23, 2),
(7, 23, 3);
...
INSERT INTO `fee` (`feeId`, `name`, `description`, `amount`, `status`) VALUES
...
INSERT INTO `invoice` (`invoiceId`, `invoiceNo`, `description`, `invoiceDate`, `patientId`, `taxAmount`, `amount`, `isPaidFor`) VALUES
(1, '1', 'Invoice for Michael Snkolongo', '2023-02-06', 2, '25.20', '277.20', 1),
(3, '2', 'Michael Sinkolongo invoice', '2023-02-06', 2, '36.40', '400.40', 1),
(5, '2300001', '2023-02-08 Invoice For Michael Sinkolongo', '2023-02-08', 2, '17.80', '195.80', 1),
(6, '2300002', 'Invoice for Michael Doctor Chali', '2023-02-08', 2, '17.60', '193.60', 1),
(7, '2300003', 'Michael Malaria Receipt', '2023-02-08', 2, '17.80', '195.80', 1);
...
CREATE TRIGGER `CreateInvoiceNoBeforeInsert` BEFORE INSERT ON `invoice` FOR EACH ROW begin 
   ...
   SET @patientName = (SELECT concat(firstName,' ',lastName) as name FROM patients WHERE patientId=New.patientId);

IF (New.description is NULL OR New.description = '') THEN
  SET New.description = CONCAT(CONVERT(CURRENT_DATE,CHARACTER),' Invoice For ', @patientName);
END IF;                 
...
(20, 1, NULL, 6, 'Pandol', '28.00', 1, NULL, 28),
(22, 1, 1, NULL, 'Consultation Fee', '150.00', 1, NULL, 150),
(23, 3, NULL, 2, 'Crosin', '26.00', 2, NULL, 52),
(24, 3, NULL, 5, 'Gelusil', '12.00', 1, NULL, 12),
(25, 3, 1, NULL, 'Consultation Fee', '150.00', 2, NULL, 300),
(26, 5, 1, NULL, 'Consultation Fee', '150.00', 1, NULL, 150),
(27, 5, NULL, 6, 'Pandol', '28.00', 1, NULL, 28),
(28, 6, NULL, 2, 'Crosin', '26.00', 1, NULL, 26),
(29, 6, 1, NULL, 'Consultation Fee', '150.00', 1, NULL, 150),
(30, 7, NULL, 6, 'Pandol', '28.00', 1, NULL, 28),
(31, 7, 1, NULL, 'Consultation Fee', '150.00', 1, NULL, 150);
...
CREATE TRIGGER `ComputeInvoiceTotalAfterDelete` AFTER DELETE ON `invoice_detail` FOR EACH ROW BEGIN
	...
	UPDATE invoice SET amount = @invoiceAmount,taxAmount=@taxAmount WHERE invoiceId = OLD.invoiceId;
...
CREATE TRIGGER `ComputeInvoiceTotalAfterInsert` AFTER INSERT ON `invoice_detail` FOR EACH ROW BEGIN
	...
	UPDATE invoice SET amount = @invoiceAmount,taxAmount=@taxAmount WHERE invoiceId = NEW.invoiceId;
...
CREATE TRIGGER `ComputeTotalsBeforeInsert` BEFORE INSERT ON `invoice_detail` FOR EACH ROW BEGIN
	...
	IF (@medicineId IS NOT NULL AND @medicineId >0 AND (@feeId IS NULL OR @feeId = 0)) THEN
	 ...
	 -- check if medicine has already been entered
	 SET @found = (SELECT count(*) FROM invoice_detail WHERE medicineId = @medicineId AND invoiceId =New.invoiceId);
	 IF(@found > 0) THEN
	 		-- raise already exists EXCEPTION
		SIGNAL SQLSTATE '45000' SET message_text = 'create_error: Medicine has already been added to invoice'; 
	 END IF;
	 -- check stock
    ...
    ELSE
	 ...
	 SET @found = (SELECT count(*) FROM invoice_detail WHERE feeId = @feeId AND invoiceId =New.invoiceId);
	 IF(@found > 0) THEN
	 		-- raise already exists EXCEPTION
		SIGNAL SQLSTATE '45000' SET message_text = 'create_error: This fee has already been added to invoice'; 
	 END IF;
	 SET @feeName = (SELECT name FROM fee WHERE feeId =@feeId);
	 SET @feeAmount = (SELECT ifnull(amount,0) FROM fee WHERE feeId =@feeId);
...
(1, 2, 'CROS12', '0000-00-00', 6, 26),
(2, 5, 'G327', '0000-00-00', 7, 12),
(3, 4, 'DOLO327', '0000-00-00', 10, 24),
(4, 6, 'NI325', '0000-00-00', 17, 28);
...
(12, 'fa fa-arrow-right', 'Issue Invoice', 'New Invoice', 'javascript:Invoice.addNewInvoiceManageinvoices_pharamacyuser({})', 1, 1, 18, NULL),
...
(16, 'fa fa-file', 'Manage Invoices', 'Manage Invoices', 'javascript:Invoice.viewInvoiceManageinvoices_pharamacyuser({})', 1, 1, 18, NULL),
(17, 'fa fa-file', 'View Receipts', 'Manage Receipts', 'javascript:Receipt.viewReceiptManagereceipts_pharamacyuser({})', 1, 1, 18, NULL);
...
INSERT INTO `patients` (`patientId`, `fileId`, `firstName`, `otherNames`, `lastName`, `address`, `contactNumber`, `dateOfBirth`, `nationality`, `status`) VALUES
...
INSERT INTO `payment_methods` (`paymentMethodId`, `name`, `description`, `status`) VALUES
...
INSERT INTO `receipt` (`receiptId`, `description`, `patientId`, `receiptNo`, `invoiceId`, `receiptDate`, `invoiceAmount`, `amountPaid`, `paymentMethodId`, `changeAmount`) VALUES
(2, 'Receipt For Michael Sinkolongo invoice', 2, '2300002', 3, '2023-02-07', NULL, '15.00', 1, '15.00'),
(3, 'Receipt For Invoice for Michael Snkolongo', 2, '2300003', 1, '2023-02-08', NULL, '300.00', 1, '22.80'),
(4, 'Receipt For Invoice for Michael Snkolongo', 2, '2300004', 1, '2023-02-08', '277.20', '400.00', 1, '122.80'),
(5, 'Receipt For Michael Sinkolongo invoice', 2, '2300005', 3, '2023-02-08', '400.40', '450.00', 1, '49.60'),
(6, 'Receipt For 2023-02-08 Invoice For Michael Sinkolongo', 2, '2300006', 5, '2023-02-08', '195.80', '200.00', 1, '4.20'),
(7, 'Receipt For Invoice for Michael Doctor Chali', 2, '2300007', 6, '2023-02-08', '193.60', '200.00', 1, '6.40'),
(8, 'Receipt For Michael Malaria Receipt', 2, '2300008', 7, '2023-02-08', '195.80', '200.00', 1, '4.20');

--
-- Triggers `receipt`
--
DELIMITER $$
CREATE TRIGGER `Create_receipt_number` BEFORE INSERT ON `receipt` FOR EACH ROW begin
				...
				SET @patientId = (SELECT patientId FROM invoice WHERE invoiceId=NEW.invoiceId);
				SET @invoiceDescription =(SELECT description FROM invoice WHERE invoiceId=NEW.invoiceId);
				SET @description = CONCAT('Receipt For ',@invoiceDescription);
				SEt @invoiceAmount = (SELECT amount FROM invoice WHERE invoiceId=NEW.invoiceId);
				SET @amountPaid = NEW.amountPaid;

				SET NEW.description = @description;
				SET NEW.patientId = @patientId;
				SET NEW.invoiceAmount = @invoiceAmount;
				...
				-- close invoice
				UPDATE invoice SET isPaidFor = 1 WHERE invoiceId = NEW.invoiceId; 
...
(15, '22104d2bb3e5d423c32936cc11a7c20e', '8bc991b42c208cad1cc8ed04725127c9f43faff88247776c75d5eb15fa227f45', 2, '2023-02-22 12:08:06');
...
--
ALTER TABLE `accounts_invoice_number_sequence`
  MODIFY `sequenceId` int(100) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;

--
-- AUTO_INCREMENT for table `accounts_receipt_number_sequence`
--
ALTER TABLE `accounts_receipt_number_sequence`
  MODIFY `sequenceId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=21;
...
--
ALTER TABLE `fee`
  MODIFY `feeId` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
ALTER TABLE `invoice`
  MODIFY `invoiceId` int(50) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
ALTER TABLE `patients`
  MODIFY `patientId` int(50) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `receipt`
  MODIFY `receiptId` int(50) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
ALTER TABLE `hospital_procedure`
  ADD CONSTRAINT `hospital_procedure_ibfk_1` FOREIGN KEY (`feeId`) REFERENCES `fee` (`feeId`),
...
ALTER TABLE `invoice`
  ...
  ADD CONSTRAINT `invoice_ibfk_3` FOREIGN KEY (`patientId`) REFERENCES `patients` (`patientId`);
...
ALTER TABLE `invoice_detail`
  ...
  ADD CONSTRAINT `invoice_detail_ibfk_2` FOREIGN KEY (`feeId`) REFERENCES `fee` (`feeId`),
...
ALTER TABLE `patient_scheme`
  ...
  ADD CONSTRAINT `patient_scheme_ibfk_3` FOREIGN KEY (`patientId`) REFERENCES `patients` (`patientId`);
...
--
ALTER TABLE `procedures_taken`
  ADD CONSTRAINT `procedures_taken_ibfk_1` FOREIGN KEY (`patientId`) REFERENCES `patients` (`patientId`),
  ADD CONSTRAINT `procedures_taken_ibfk_2` FOREIGN KEY (`patientId`) REFERENCES `patients` (`patientId`),
...
--
ALTER TABLE `receipt`
  ADD CONSTRAINT `receipt_ibfk_1` FOREIGN KEY (`patientId`) REFERENCES `patients` (`patientId`),
  ADD CONSTRAINT `receipt_ibfk_2` FOREIGN KEY (`paymentMethodId`) REFERENCES `payment_methods` (`paymentMethodId`),
  ADD CONSTRAINT `receipt_ibfk_3` FOREIGN KEY (`invoiceId`) REFERENCES `invoice` (`invoiceId`);

--
-- Constraints for table `regular_checkups`
--
ALTER TABLE `regular_checkups`
  ADD CONSTRAINT `regular_checkups_ibfk_1` FOREIGN KEY (`patientId`) REFERENCES `patients` (`patientId`),
