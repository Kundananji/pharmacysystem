...
/**
* function to insert single object of type Yesno into database
* @param object instance of type Yesno
...
*/
  public function insert($yesno){
    ...
    try{
      ...
      $insertedYesno = $this->getFields($row);
        $stmt->close();
        return $insertedYesno;
      }
      $stmt->close();
      return null;
    }
...
/**
* function to update single object of type Yesno
* @param object instance of type Yesno
  ...
  public function update($yesno){
    ...
    try{
      ...
      while($row=$result->fetch_assoc()){
        $updatedYesno = $this->getFields($row);
        $stmt->close();
        return $updatedYesno;
      }
      $stmt->close();
...
/**
* function to select array of object of type Yesno from database through the provided search term
* @return array of objects of type Yesno
*/
  public function search($term,$searchFields,$extra=null,$pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM yesno WHERE (".implode(' OR ',$whereClause).") ".($extra!=null?("AND ".$extra):'')." $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
      while($row=$result->fetch_assoc()){
        $yesno = $this->getFields($row);
        $objects[]=$yesno;
      }
      $stmt->close();
      return $objects;
...
/**
* function to select array of object of type Yesno from database
* @return array of objects of type Yesno
*/
  public function selectAll($pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM yesno $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
...
*/
  public function selectBy($fields,$values,$types,$orderByFields=array(),$pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM yesno  WHERE ".implode(" AND ",$whereVars) .(sizeof($orderByFields)>0?("ORDER BY ".implode(",",$orderByFields)):"")." $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
...
*/
  public function selectByWhereClause($whereClause){
    try{
      $sql="SELECT * FROM yesno  WHERE ".$whereClause;
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
