...
/**
* function to insert single object of type Status into database
* @param object instance of type Status
...
*/
  public function insert($status){
    ...
    try{
      ...
      $insertedStatus = $this->getFields($row);
        $stmt->close();
...
/**
* function to update single object of type Status
* @param object instance of type Status
  ...
  public function update($status){
    ...
    try{
      ...
      while($row=$result->fetch_assoc()){
        $updatedStatus = $this->getFields($row);
        $stmt->close();
...
/**
* function to select array of object of type Status from database through the provided search term
* @return array of objects of type Status
*/
  public function search($term,$searchFields,$extra=null,$pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM status WHERE (".implode(' OR ',$whereClause).") ".($extra!=null?("AND ".$extra):'')." $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
      while($row=$result->fetch_assoc()){
        $status = $this->getFields($row);
        $objects[]=$status;
      }
      $stmt->close();
      return $objects;
...
/**
* function to select array of object of type Status from database
* @return array of objects of type Status
*/
  public function selectAll($pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM status $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
...
*/
  public function selectBy($fields,$values,$types,$orderByFields=array(),$pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM status  WHERE ".implode(" AND ",$whereVars) .(sizeof($orderByFields)>0?("ORDER BY ".implode(",",$orderByFields)):"")." $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
...
*/
  public function selectByWhereClause($whereClause){
    try{
      $sql="SELECT * FROM status  WHERE ".$whereClause;
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
