...
/**
* function to insert single object of type Profile into database
* @param object instance of type Profile
...
*/
  public function insert($profile){
    ...
    try{
      $sql="INSERT INTO _profile(`id`,`name`,`description`,`isActive`,`isDefault`) VALUES(?,?,?,?,?)";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->bind_param("issii",$id,$name,$description,$isActive,$isDefault);
      $stmt->execute();
      $stmt->store_result();
      $inserted = $stmt->insert_id;
      $stmt->close();

      $sql="SELECT * FROM _profile WHERE `id`=?";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->bind_param("i",$inserted);
      $stmt->execute();
      $result = $stmt->get_result();
      if($row=$result->fetch_assoc()){
      $insertedProfile = $this->getFields($row);
        $stmt->close();
...
/**
* function to update single object of type Profile
* @param object instance of type Profile
...
*/
  public function update($profile){
    ...
    $isDefault=  $profile->getIsDefault();
    try{
      $sql="UPDATE _profile SET `name`=?,`description`=?,`isActive`=?,`isDefault`=? WHERE id =?";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->bind_param("ssiii",$name,$description,$isActive,$isDefault,$id);
      $stmt->execute();
      $stmt->close();
      ...
      while($row=$result->fetch_assoc()){
        $updatedProfile = $this->getFields($row);
        $stmt->close();
...
/**
* function to select array of object of type Profile from database through the provided search term
* @return array of objects of type Profile
*/
  public function search($term,$searchFields,$extra=null,$pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM _profile WHERE (".implode(' OR ',$whereClause).") ".($extra!=null?("AND ".$extra):'')." $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
      while($row=$result->fetch_assoc()){
        $profile = $this->getFields($row);
        $objects[]=$profile;
      }
      $stmt->close();
      return $objects;
...
/**
* function to select array of object of type Profile from database
* @return array of objects of type Profile
*/
  public function selectAll($pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM _profile $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
...
/**
* function to select single instance of object of type Profile from database
...
*/
  public function selectBy($fields,$values,$types,$orderByFields=array(),$pageSize=0,$pageNo=null){
    ...
    try{
      $sql="SELECT * FROM _profile  WHERE ".implode(" AND ",$whereVars) .(sizeof($orderByFields)>0?("ORDER BY ".implode(",",$orderByFields)):"")." $limitClause";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
...
/**
* function to delete a single instance of object of type Profile from database
* @return boolean
*/
  public function delete($profileId){
    try{
      $sql="DELETE FROM `_profile` WHERE `id`=?";
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->bind_param("i",$profileId);
      $stmt->execute();
...
*/
  public function selectByWhereClause($whereClause){
    try{
      $sql="SELECT * FROM _profile  WHERE ".$whereClause;
      $stmt=Database::getConnection()->prepare($sql);
      $stmt->execute();
      $result = $stmt->get_result();
      $objects = array();
