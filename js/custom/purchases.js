  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Purchases.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_purchases_id",tableId);
    localStorage.setItem("table_purchases",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-purchases-supplier-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-purchases-invoice-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-purchases-voucher-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-purchases-purchase-date' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-purchases-total-amount' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-purchases-payment-status' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_purchases",rowCount-1);
  }
  localStorage.setItem("table_purchases_id",tableId);
}
...
let addNewPurchases=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/purchases-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormPurchases = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-purchases');
  ...
  $.ajax({
    url: "scripts/submit-purchases-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
/*
**/
 let deletePurchases = (purchasesId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-purchases-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-purchases').off().on('click',()=>{
    viewPurchases();
  });

  $('#add-new-purchases').on('click',()=>{
    addNewPurchases();
  });
});

 return {
   addNewPurchases:addNewPurchases,
  ...
  viewPurchases:viewPurchases,
   submitFormPurchases : submitFormPurchases, 
