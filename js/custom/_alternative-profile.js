let AlternativeProfile  = (function($) {
"use strict";
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table__alternative_profile_id",tableId);
    localStorage.setItem("table__alternative_profile",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-_alternative-profile-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-_alternative-profile-user-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-_alternative-profile-profile-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table__alternative_profile",rowCount-1);
  }
  localStorage.setItem("table__alternative_profile_id",tableId);
}
...
/**
*
*function to view items for permission User Profile
*
 **/
let viewAlternativeProfileUserprofiles_admin=(data)=>{
...
let addNewAlternativeProfileUserprofiles_admin=(data)=>{
 ...
 let submitFormAlternativeProfileUserprofiles_admin = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-_alternative-profile');
  ...
  for(let i=0; i<elements.length; i++){
    ...
    console.log('_alternative_profile',element);
  };
  console.log('_alternative_profile',data);
...
/*
**/
 let deleteAlternativeProfileUserprofiles_admin = (alternativeProfileId, args) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-_alternative-profile-item.php",
      type: "post",
      dataType: "json",
...
let viewAlternativeProfile=( id)=>{
  let viewInModal = $('#link-view-_alternative-profile').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-_alternative-profile.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
        divPageContent.html(data);
      $('#table-_alternative-profile').DataTable({});
    }
  });
}

let addNewAlternativeProfile=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/_alternative-profile-form.php",
    type: "get",
    data:data,
    success: (data)=>{
      $('#dataInputModalBody').html(data);
 ...
 let submitFormAlternativeProfile = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-_alternative-profile');
  let elements = oForm.elements;
  let data = {};
  ...
  $('#form-submit-button').prop('disabled', true);
  $.ajax({
    url: "scripts/submit-_alternative-profile-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
/*
**/
 let deleteAlternativeProfile = (alternativeProfileId) =>{
  swal({
    title: "Are you sure you want to delete this item?",
    text: "If you are sure you want to delete; proceed and click okay below. Otherwise, cancel to abort.",
    type: "warning",
...
$(document).ready(()=>{
  $('#link-view-_alternative-profile').off().on('click',()=>{
    viewAlternativeProfile();
  });

  $('#add-new-_alternative-profile').on('click',()=>{
    addNewAlternativeProfile();
  });
});

 return {
   addNewAlternativeProfile:addNewAlternativeProfile,
   deleteAlternativeProfile:deleteAlternativeProfile,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewAlternativeProfile:viewAlternativeProfile,
   submitFormAlternativeProfile : submitFormAlternativeProfile, 
   uploadFile : uploadFile, 
   deleteFile : deleteFile, 
   viewAlternativeProfileUserprofiles_admin : viewAlternativeProfileUserprofiles_admin, 
    addNewAlternativeProfileUserprofiles_admin :  addNewAlternativeProfileUserprofiles_admin, 
   submitFormAlternativeProfileUserprofiles_admin : submitFormAlternativeProfileUserprofiles_admin, 
   deleteAlternativeProfileUserprofiles_admin : deleteAlternativeProfileUserprofiles_admin, 
}
