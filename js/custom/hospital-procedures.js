let HospitalProcedures  = (function($) {
"use strict";
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_hospital_procedures_id",tableId);
    localStorage.setItem("table_hospital_procedures",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-hospital-procedures-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedures-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedures-description' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedures-fee' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_hospital_procedures",rowCount-1);
  }
  localStorage.setItem("table_hospital_procedures_id",tableId);
}

let divPageContent = $('#page-content');
let viewHospitalProcedures=( id)=>{
  let viewInModal = $('#link-view-hospital-procedures').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-hospital-procedures.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-hospital-procedures').DataTable({});
    }
  });
}

let addNewHospitalProcedures=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/hospital-procedures-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormHospitalProcedures = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-hospital-procedures');
...
/*
**/
 let deleteHospitalProcedures = (hospitalProceduresId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-hospital-procedures-item.php",
      type: "post",
      dataType: "json",
      data:{hospitalProceduresId: hospitalProceduresId},
      success: (data)=>{
        if(data.status == "success"){
          viewHospitalProcedures();
          swal(data.title,data.message,'success');
      }else{
          swal(data.title,data.message,'warning');
        }
...
$(document).ready(()=>{
  $('#link-view-hospital-procedures').off().on('click',()=>{
    viewHospitalProcedures();
  });

  $('#add-new-hospital-procedures').on('click',()=>{
    addNewHospitalProcedures();
  });
});

 return {
   addNewHospitalProcedures:addNewHospitalProcedures,
   deleteHospitalProcedures:deleteHospitalProcedures,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewHospitalProcedures:viewHospitalProcedures,
   submitFormHospitalProcedures : submitFormHospitalProcedures, 
