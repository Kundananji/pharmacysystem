  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="RegularCheckups.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_regular_checkups_id",tableId);
    localStorage.setItem("table_regular_checkups",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-regular-checkups-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-regular-checkups-patient-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  ...
  id = 'input-regular-checkups-blood-pressure' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-regular-checkups-weight' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-regular-checkups-other' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  ...
  if(incrementRowCount){
    localStorage.setItem("table_regular_checkups",rowCount-1);
  }
  localStorage.setItem("table_regular_checkups_id",tableId);
}

let divPageContent = $('#page-content');
let viewRegularCheckups=( id)=>{
  let viewInModal = $('#link-view-regular-checkups').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-regular-checkups.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-regular-checkups').DataTable({});
    }
  });
}

let addNewRegularCheckups=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/regular-checkups-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormRegularCheckups = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-regular-checkups');
...
/*
**/
 let deleteRegularCheckups = (regularCheckupsId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-regular-checkups-item.php",
      type: "post",
      dataType: "json",
      data:{regularCheckupsId: regularCheckupsId},
      success: (data)=>{
        if(data.status == "success"){
          viewRegularCheckups();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-regular-checkups').off().on('click',()=>{
    viewRegularCheckups();
  });

  $('#add-new-regular-checkups').on('click',()=>{
    addNewRegularCheckups();
  });
});

 return {
   addNewRegularCheckups:addNewRegularCheckups,
   deleteRegularCheckups:deleteRegularCheckups,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewRegularCheckups:viewRegularCheckups,
   submitFormRegularCheckups : submitFormRegularCheckups, 
