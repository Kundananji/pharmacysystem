let AccountsReceiptNumberSequence  = (function($) {
"use strict";
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_accounts_receipt_number_sequence_id",tableId);
    localStorage.setItem("table_accounts_receipt_number_sequence",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-accounts-receipt-number-sequence-sequence-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-accounts-receipt-number-sequence-year' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-accounts-receipt-number-sequence-receipt-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_accounts_receipt_number_sequence",rowCount-1);
  }
  localStorage.setItem("table_accounts_receipt_number_sequence_id",tableId);
}

let divPageContent = $('#page-content');
let viewAccountsReceiptNumberSequence=( sequenceId)=>{
  let viewInModal = $('#link-view-accounts-receipt-number-sequence').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-accounts-receipt-number-sequence.php",
    type: "post",
    data: { 
    ...
    success: (data)=>{
      ...
      $('#table-accounts-receipt-number-sequence').DataTable({});
    }
  });
}

let addNewAccountsReceiptNumberSequence=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/accounts-receipt-number-sequence-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormAccountsReceiptNumberSequence = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-accounts-receipt-number-sequence');
  ...
  $('#dataInputModal').modal('hide');
        viewAccountsReceiptNumberSequence(args);
...
/*
**/
 let deleteAccountsReceiptNumberSequence = (accountsReceiptNumberSequenceId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-accounts-receipt-number-sequence-item.php",
      type: "post",
      dataType: "json",
      data:{accountsReceiptNumberSequenceId: accountsReceiptNumberSequenceId},
      success: (data)=>{
        if(data.status == "success"){
          viewAccountsReceiptNumberSequence();
          swal(data.title,data.message,'success');
      }else{
          swal(data.title,data.message,'warning');
        }
...
$(document).ready(()=>{
  $('#link-view-accounts-receipt-number-sequence').off().on('click',()=>{
    viewAccountsReceiptNumberSequence();
  });

  $('#add-new-accounts-receipt-number-sequence').on('click',()=>{
    addNewAccountsReceiptNumberSequence();
  });
});

 return {
   addNewAccountsReceiptNumberSequence:addNewAccountsReceiptNumberSequence,
   deleteAccountsReceiptNumberSequence:deleteAccountsReceiptNumberSequence,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewAccountsReceiptNumberSequence:viewAccountsReceiptNumberSequence,
   submitFormAccountsReceiptNumberSequence : submitFormAccountsReceiptNumberSequence, 
