  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Staff.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_staff_id",tableId);
    localStorage.setItem("table_staff",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-staff-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-title' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-position' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-phone-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-address' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-nationaility' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-status' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-staff-man-no' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_staff",rowCount-1);
  }
  localStorage.setItem("table_staff_id",tableId);
}
...
let viewStaff=( id)=>{
  let viewInModal = $('#link-view-staff').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-staff.php",
    type: "post",
    data: { 
      id:id
     },
...
let addNewStaff=(data)=>{
 ...
 let submitFormStaff = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-staff');
  ...
  $.ajax({
    url: "scripts/submit-staff-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
**/
 let deleteStaff = (staffId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-staff-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-staff').off().on('click',()=>{
    viewStaff();
  });

  $('#add-new-staff').on('click',()=>{
    addNewStaff();
  });
});
 ...
 return {
  ...
  viewStaff:viewStaff,
   submitFormStaff : submitFormStaff, 
