  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="InvoiceSettings.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_invoice_settings_id",tableId);
    localStorage.setItem("table_invoice_settings",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-invoice-settings-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-invoice-settings-terms' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-invoice-settings-tax-rate' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_invoice_settings",rowCount-1);
  }
  localStorage.setItem("table_invoice_settings_id",tableId);
}

let divPageContent = $('#page-content');
let viewInvoiceSettings=( id)=>{
  let viewInModal = $('#link-view-invoice-settings').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-invoice-settings.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-invoice-settings').DataTable({});
    }
  });
}

let addNewInvoiceSettings=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/invoice-settings-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormInvoiceSettings = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-invoice-settings');
...
/*
**/
 let deleteInvoiceSettings = (invoiceSettingsId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-invoice-settings-item.php",
      type: "post",
      dataType: "json",
      data:{invoiceSettingsId: invoiceSettingsId},
      success: (data)=>{
        if(data.status == "success"){
          viewInvoiceSettings();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-invoice-settings').off().on('click',()=>{
    viewInvoiceSettings();
  });

  $('#add-new-invoice-settings').on('click',()=>{
    addNewInvoiceSettings();
  });
});

 return {
   addNewInvoiceSettings:addNewInvoiceSettings,
   deleteInvoiceSettings:deleteInvoiceSettings,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewInvoiceSettings:viewInvoiceSettings,
   submitFormInvoiceSettings : submitFormInvoiceSettings, 
