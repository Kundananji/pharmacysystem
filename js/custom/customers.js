  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Customers.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_customers_id",tableId);
    localStorage.setItem("table_customers",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-customers-contact-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  ...
  if(incrementRowCount){
    localStorage.setItem("table_customers",rowCount-1);
  }
  localStorage.setItem("table_customers_id",tableId);
}
...
let addNewCustomers=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/customers-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormCustomers = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-customers');
  ...
  $.ajax({
    url: "scripts/submit-customers-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
/*
**/
 let deleteCustomers = (customersId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-customers-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-customers').off().on('click',()=>{
    viewCustomers();
  });

  $('#add-new-customers').on('click',()=>{
    addNewCustomers();
  });
});
 ...
 return {
  ...
  viewCustomers:viewCustomers,
   submitFormCustomers : submitFormCustomers, 
