  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Suppliers.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_suppliers_id",tableId);
    localStorage.setItem("table_suppliers",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-suppliers-i-d' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-suppliers-n-a-m-e' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-suppliers-e-m-a-i-l' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-suppliers-contact-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-suppliers-a-d-d-r-e-s-s' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_suppliers",rowCount-1);
  }
  localStorage.setItem("table_suppliers_id",tableId);
}

let divPageContent = $('#page-content');
let viewSuppliers=( ID)=>{
  let viewInModal = $('#link-view-suppliers').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-suppliers.php",
    type: "post",
    data: { 
      ID:ID
     },
...
let addNewSuppliers=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/suppliers-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormSuppliers = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-suppliers');
  ...
  $.ajax({
    url: "scripts/submit-suppliers-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
/*
**/
 let deleteSuppliers = (suppliersId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-suppliers-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-suppliers').off().on('click',()=>{
    viewSuppliers();
  });

  $('#add-new-suppliers').on('click',()=>{
    addNewSuppliers();
  });
});
 ...
 return {
  ...
  viewSuppliers:viewSuppliers,
   submitFormSuppliers : submitFormSuppliers, 
