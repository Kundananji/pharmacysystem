let HospitalProcedure  = (function($) {
"use strict";
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_hospital_procedure_id",tableId);
    localStorage.setItem("table_hospital_procedure",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-hospital-procedure-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedure-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedure-description' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedure-department-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-hospital-procedure-fee-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_hospital_procedure",rowCount-1);
  }
  localStorage.setItem("table_hospital_procedure_id",tableId);
}

let divPageContent = $('#page-content');
let viewHospitalProcedure=( id)=>{
  let viewInModal = $('#link-view-hospital-procedure').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-hospital-procedure.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-hospital-procedure').DataTable({});
    }
  });
}

let addNewHospitalProcedure=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/hospital-procedure-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormHospitalProcedure = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-hospital-procedure');
...
/*
**/
 let deleteHospitalProcedure = (hospitalProcedureId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-hospital-procedure-item.php",
      type: "post",
      dataType: "json",
      data:{hospitalProcedureId: hospitalProcedureId},
      success: (data)=>{
        if(data.status == "success"){
          viewHospitalProcedure();
          swal(data.title,data.message,'success');
      }else{
          swal(data.title,data.message,'warning');
        }
...
$(document).ready(()=>{
  $('#link-view-hospital-procedure').off().on('click',()=>{
    viewHospitalProcedure();
  });

  $('#add-new-hospital-procedure').on('click',()=>{
    addNewHospitalProcedure();
  });
});

 return {
   addNewHospitalProcedure:addNewHospitalProcedure,
   deleteHospitalProcedure:deleteHospitalProcedure,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewHospitalProcedure:viewHospitalProcedure,
   submitFormHospitalProcedure : submitFormHospitalProcedure, 
