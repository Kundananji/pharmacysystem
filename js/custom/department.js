  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Department.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_department_id",tableId);
    localStorage.setItem("table_department",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-department-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-department-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-department-description' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_department",rowCount-1);
  }
  localStorage.setItem("table_department_id",tableId);
}

let divPageContent = $('#page-content');
let viewDepartment=( id)=>{
  let viewInModal = $('#link-view-department').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-department.php",
    type: "post",
    data: { 
      id:id
     },
...
let addNewDepartment=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/department-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormDepartment = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-department');
  ...
  $.ajax({
    url: "scripts/submit-department-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
**/
 let deleteDepartment = (departmentId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-department-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-department').off().on('click',()=>{
    viewDepartment();
  });

  $('#add-new-department').on('click',()=>{
    addNewDepartment();
  });
});
 ...
 return {
  ...
  viewDepartment:viewDepartment,
   submitFormDepartment : submitFormDepartment, 
