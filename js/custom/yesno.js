  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Yesno.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_yesno_id",tableId);
    localStorage.setItem("table_yesno",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-yesno-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-yesno-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_yesno",rowCount-1);
  }
  localStorage.setItem("table_yesno_id",tableId);
}
...
let viewYesno=( id)=>{
  let viewInModal = $('#link-view-yesno').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-yesno.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-yesno').DataTable({});
    }
  });
}

let addNewYesno=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/yesno-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormYesno = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-yesno');
...
**/
 let deleteYesno = (yesnoId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-yesno-item.php",
      type: "post",
      dataType: "json",
      data:{yesnoId: yesnoId},
      success: (data)=>{
        if(data.status == "success"){
          viewYesno();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-yesno').off().on('click',()=>{
    viewYesno();
  });

  $('#add-new-yesno').on('click',()=>{
    addNewYesno();
  });
});

 return {
   addNewYesno:addNewYesno,
   deleteYesno:deleteYesno,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewYesno:viewYesno,
   submitFormYesno : submitFormYesno, 
