  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Medicines.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_medicines_id",tableId);
    localStorage.setItem("table_medicines",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-medicines-generic-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-medicines-supplier-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_medicines",rowCount-1);
  }
  localStorage.setItem("table_medicines_id",tableId);
}
...
let addNewMedicines=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/medicines-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormMedicines = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-medicines');
  ...
  $.ajax({
    url: "scripts/submit-medicines-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
/*
**/
 let deleteMedicines = (medicinesId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-medicines-item.php",
      type: "post",
      dataType: "json",
      data:{medicinesId: medicinesId},
      success: (data)=>{
        if(data.status == "success"){
          viewMedicines();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-medicines').off().on('click',()=>{
    viewMedicines();
  });

  $('#add-new-medicines').on('click',()=>{
    addNewMedicines();
  });
});
