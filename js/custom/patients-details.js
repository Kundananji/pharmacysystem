  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="PatientsDetails.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_patients_details_id",tableId);
    localStorage.setItem("table_patients_details",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-patients-details-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-file-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-first-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-last-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-address' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-contact-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-date-of-birth' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-nationality' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patients-details-status' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_patients_details",rowCount-1);
  }
  localStorage.setItem("table_patients_details_id",tableId);
}

let divPageContent = $('#page-content');
let viewPatientsDetails=( id)=>{
  let viewInModal = $('#link-view-patients-details').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-patients-details.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-patients-details').DataTable({});
    }
  });
}

let addNewPatientsDetails=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/patients-details-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormPatientsDetails = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-patients-details');
...
/*
**/
 let deletePatientsDetails = (patientsDetailsId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-patients-details-item.php",
      type: "post",
      dataType: "json",
      data:{patientsDetailsId: patientsDetailsId},
      success: (data)=>{
        if(data.status == "success"){
          viewPatientsDetails();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-patients-details').off().on('click',()=>{
    viewPatientsDetails();
  });

  $('#add-new-patients-details').on('click',()=>{
    addNewPatientsDetails();
  });
});

 return {
   addNewPatientsDetails:addNewPatientsDetails,
   deletePatientsDetails:deletePatientsDetails,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewPatientsDetails:viewPatientsDetails,
   submitFormPatientsDetails : submitFormPatientsDetails, 
