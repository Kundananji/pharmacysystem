  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="UserTokens.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table__user_tokens_id",tableId);
    localStorage.setItem("table__user_tokens",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-_user-tokens-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-_user-tokens-selector' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-_user-tokens-hashed-validator' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-_user-tokens-user-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-_user-tokens-expiry' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table__user_tokens",rowCount-1);
  }
  localStorage.setItem("table__user_tokens_id",tableId);
}

let divPageContent = $('#page-content');
let viewUserTokens=( id)=>{
  let viewInModal = $('#link-view-_user-tokens').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-_user-tokens.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-_user-tokens').DataTable({});
    }
  });
}

let addNewUserTokens=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/_user-tokens-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormUserTokens = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-_user-tokens');
...
/*
**/
 let deleteUserTokens = (userTokensId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-_user-tokens-item.php",
      type: "post",
      dataType: "json",
      data:{userTokensId: userTokensId},
      success: (data)=>{
        if(data.status == "success"){
          viewUserTokens();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-_user-tokens').off().on('click',()=>{
    viewUserTokens();
  });

  $('#add-new-_user-tokens').on('click',()=>{
    addNewUserTokens();
  });
});

 return {
   addNewUserTokens:addNewUserTokens,
  ...
  viewUserTokens:viewUserTokens,
   submitFormUserTokens : submitFormUserTokens, 
