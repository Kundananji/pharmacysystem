  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="ReceiptDetail.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_receipt_detail_id",tableId);
    localStorage.setItem("table_receipt_detail",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-receipt-detail-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-receipt-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-item' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-description' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-quantity' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-unit-price' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-total-amount' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-invoice-detail-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-fee-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-medicine-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-receipt-detail-discount' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_receipt_detail",rowCount-1);
  }
  localStorage.setItem("table_receipt_detail_id",tableId);
}

let divPageContent = $('#page-content');
let viewReceiptDetail=( id)=>{
  let viewInModal = $('#link-view-receipt-detail').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-receipt-detail.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-receipt-detail').DataTable({});
    }
  });
}

let addNewReceiptDetail=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/receipt-detail-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormReceiptDetail = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-receipt-detail');
...
/*
**/
 let deleteReceiptDetail = (receiptDetailId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-receipt-detail-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-receipt-detail').off().on('click',()=>{
    viewReceiptDetail();
  });

  $('#add-new-receipt-detail').on('click',()=>{
    addNewReceiptDetail();
  });
});

 return {
   addNewReceiptDetail:addNewReceiptDetail,
  ...
  viewReceiptDetail:viewReceiptDetail,
   submitFormReceiptDetail : submitFormReceiptDetail, 
