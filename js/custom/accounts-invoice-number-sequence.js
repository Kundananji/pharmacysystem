let AccountsInvoiceNumberSequence  = (function($) {
"use strict";
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_accounts_invoice_number_sequence_id",tableId);
    localStorage.setItem("table_accounts_invoice_number_sequence",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-accounts-invoice-number-sequence-sequence-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-accounts-invoice-number-sequence-year' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-accounts-invoice-number-sequence-invoice-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_accounts_invoice_number_sequence",rowCount-1);
  }
  localStorage.setItem("table_accounts_invoice_number_sequence_id",tableId);
}

let divPageContent = $('#page-content');
let viewAccountsInvoiceNumberSequence=( sequenceId)=>{
  let viewInModal = $('#link-view-accounts-invoice-number-sequence').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-accounts-invoice-number-sequence.php",
    type: "post",
    data: { 
    ...
    success: (data)=>{
      ...
      $('#table-accounts-invoice-number-sequence').DataTable({});
    }
  });
}

let addNewAccountsInvoiceNumberSequence=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/accounts-invoice-number-sequence-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormAccountsInvoiceNumberSequence = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-accounts-invoice-number-sequence');
  ...
  $('#dataInputModal').modal('hide');
        viewAccountsInvoiceNumberSequence(args);
...
/*
**/
 let deleteAccountsInvoiceNumberSequence = (accountsInvoiceNumberSequenceId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-accounts-invoice-number-sequence-item.php",
      type: "post",
      dataType: "json",
      data:{accountsInvoiceNumberSequenceId: accountsInvoiceNumberSequenceId},
      success: (data)=>{
        if(data.status == "success"){
          viewAccountsInvoiceNumberSequence();
          swal(data.title,data.message,'success');
      }else{
          swal(data.title,data.message,'warning');
        }
...
$(document).ready(()=>{
  $('#link-view-accounts-invoice-number-sequence').off().on('click',()=>{
    viewAccountsInvoiceNumberSequence();
  });

  $('#add-new-accounts-invoice-number-sequence').on('click',()=>{
    addNewAccountsInvoiceNumberSequence();
  });
});

 return {
   addNewAccountsInvoiceNumberSequence:addNewAccountsInvoiceNumberSequence,
   deleteAccountsInvoiceNumberSequence:deleteAccountsInvoiceNumberSequence,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewAccountsInvoiceNumberSequence:viewAccountsInvoiceNumberSequence,
   submitFormAccountsInvoiceNumberSequence : submitFormAccountsInvoiceNumberSequence, 
