  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="JobPosition.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_job_position_id",tableId);
    localStorage.setItem("table_job_position",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-job-position-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-job-position-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-job-position-description' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_job_position",rowCount-1);
  }
  localStorage.setItem("table_job_position_id",tableId);
}

let divPageContent = $('#page-content');
let viewJobPosition=( id)=>{
  let viewInModal = $('#link-view-job-position').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-job-position.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-job-position').DataTable({});
    }
  });
}

let addNewJobPosition=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/job-position-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormJobPosition = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-job-position');
...
/*
**/
 let deleteJobPosition = (jobPositionId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-job-position-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-job-position').off().on('click',()=>{
    viewJobPosition();
  });

  $('#add-new-job-position').on('click',()=>{
    addNewJobPosition();
  });
});

 return {
   addNewJobPosition:addNewJobPosition,
   deleteJobPosition:deleteJobPosition,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewJobPosition:viewJobPosition,
   submitFormJobPosition : submitFormJobPosition, 
