  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Status.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_status_id",tableId);
    localStorage.setItem("table_status",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-status-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-status-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_status",rowCount-1);
  }
  localStorage.setItem("table_status_id",tableId);
}
...
let viewStatus=( id)=>{
  let viewInModal = $('#link-view-status').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-status.php",
    type: "post",
    data: { 
      id:id
     },
...
let addNewStatus=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/status-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormStatus = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-status');
  ...
  $.ajax({
    url: "scripts/submit-status-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
**/
 let deleteStatus = (statusId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-status-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-status').off().on('click',()=>{
    viewStatus();
  });

  $('#add-new-status').on('click',()=>{
    addNewStatus();
  });
});
