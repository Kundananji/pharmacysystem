  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Privilege.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_privilege_id",tableId);
    localStorage.setItem("table_privilege",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-privilege-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-privilege-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-privilege-profile-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_privilege",rowCount-1);
  }
  localStorage.setItem("table_privilege_id",tableId);
}
...
/**
*
*function to view items for permission Privilege
*
 **/
let viewPrivilegePrivileges_admin=(data)=>{
//assign global fields current values
  loader(divPageContent);
  $.ajax({
    url: "pages/view-privilege-privileges_admin.php",
    type: "post",
    data:data,
    success: (data)=>{
      divPageContent.html(data);
      $('#table-privilege--privileges_admin').DataTable({});
    }
  });
  };
let addNewPrivilegePrivileges_admin=(data)=>{
  ...
  $.ajax({
    url: "forms/privilege-privileges_admin-form.php",
    type: "get",
    data: data, 
    success: (data)=>{
 ...
 let submitFormPrivilegePrivileges_admin = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-privilege');
  ...
  $.ajax({
    url: "scripts/submit-privilege-privileges_admin-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (resp)=>{
...
/*
**/
 let deletePrivilegePrivileges_admin = (privilegeId, args) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-privilege-item.php",
      type: "post",
      dataType: "json",
...
let viewPrivilege=( id)=>{
  let viewInModal = $('#link-view-privilege').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-privilege.php",
    type: "post",
    data: { 
      id:id
     },
...
let addNewPrivilege=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/privilege-form.php",
    type: "get",
    data:data,
    success: (data)=>{
      $('#dataInputModalBody').html(data);
 ...
 let submitFormPrivilege = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-privilege');
  let elements = oForm.elements;
  let data = {};
  ...
  $('#form-submit-button').prop('disabled', true);
  $.ajax({
    url: "scripts/submit-privilege-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
/*
**/
 let deletePrivilege = (privilegeId) =>{
  swal({
    title: "Are you sure you want to delete this item?",
    text: "If you are sure you want to delete; proceed and click okay below. Otherwise, cancel to abort.",
    type: "warning",
...
$(document).ready(()=>{
  $('#link-view-privilege').off().on('click',()=>{
    viewPrivilege();
  });

  $('#add-new-privilege').on('click',()=>{
    addNewPrivilege();
  });
});
 ...
 return {
  ...
  viewPrivilege:viewPrivilege,
   submitFormPrivilege : submitFormPrivilege, 
   uploadFile : uploadFile, 
   deleteFile : deleteFile, 
   viewPrivilegePrivileges_admin : viewPrivilegePrivileges_admin, 
    addNewPrivilegePrivileges_admin :  addNewPrivilegePrivileges_admin, 
   submitFormPrivilegePrivileges_admin : submitFormPrivilegePrivileges_admin, 
   deletePrivilegePrivileges_admin : deletePrivilegePrivileges_admin, 
}
