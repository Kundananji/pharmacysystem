  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="PatientScheme.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_patient_scheme_id",tableId);
    localStorage.setItem("table_patient_scheme",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-patient-scheme-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patient-scheme-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patient-scheme-description' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patient-scheme-patient-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patient-scheme-insurance-provider-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-patient-scheme-status' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  if(incrementRowCount){
    localStorage.setItem("table_patient_scheme",rowCount-1);
  }
  localStorage.setItem("table_patient_scheme_id",tableId);
}

let divPageContent = $('#page-content');
let viewPatientScheme=( id)=>{
  let viewInModal = $('#link-view-patient-scheme').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-patient-scheme.php",
    type: "post",
    data: { 
      id:id
     },
    success: (data)=>{
      ...
      $('#table-patient-scheme').DataTable({});
    }
  });
}

let addNewPatientScheme=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/patient-scheme-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormPatientScheme = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-patient-scheme');
...
/*
**/
 let deletePatientScheme = (patientSchemeId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-patient-scheme-item.php",
      type: "post",
      dataType: "json",
      data:{patientSchemeId: patientSchemeId},
      success: (data)=>{
        if(data.status == "success"){
          viewPatientScheme();
          swal(data.title,data.message,'success');
...
$(document).ready(()=>{
  $('#link-view-patient-scheme').off().on('click',()=>{
    viewPatientScheme();
  });

  $('#add-new-patient-scheme').on('click',()=>{
    addNewPatientScheme();
  });
});

 return {
   addNewPatientScheme:addNewPatientScheme,
   deletePatientScheme:deletePatientScheme,
  removeFormTableRow:removeFormTableRow,
  addFormTableRow:addFormTableRow,
  viewPatientScheme:viewPatientScheme,
   submitFormPatientScheme : submitFormPatientScheme, 
