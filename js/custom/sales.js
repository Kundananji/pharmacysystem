  ...
  $.ajax({
    ...
    success: function(response){
      if(response.status=="success"){
        ...
        var output ='<div id="" class="row m-3"><div class="col-sm-12 col-md-2 col-lg-2"><img src="'+url+'" class="rounded" style="width:100%"/></div><div class="col-sm-12 col-md-10 col-lg-10"><a href="javascript:void(0)" onclick="Sales.deleteFile(\''+typeOfFile+'\',\''+fieldId+'\',\''+fieldName+'\')" class="btn"><i class="fa fa-trash"></i> Delete Image</a></div></div>';
...
let removeFormTableRow=(tableId,obj)=>{
  ...
  localStorage.setItem("table_sales_id",tableId);
    localStorage.setItem("table_sales",(rows.length-1));
}
...
let addFormTableRow=(tableId,incrementRowCount=true)=>{
  ...
  id = 'input-sales-customer-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-sales-invoice-number' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-sales-medicine-name' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-sales-batch-id' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  id = 'input-sales-expiry-date' +'_row_' + i;
  storedValue = localStorage.getItem(id);
  if(storedValue){
    $('#'+id).val(storedValue);
  }
  ...
  if(incrementRowCount){
    localStorage.setItem("table_sales",rowCount-1);
  }
  localStorage.setItem("table_sales_id",tableId);
}
...
let viewSales=( )=>{
  let viewInModal = $('#link-view-sales').data('viewInModal'); 
    loader(divPageContent);
  $.ajax({
    url: "pages/view-sales.php",
    type: "post",
    data: { 
     },
    success: (data)=>{
...
let addNewSales=(data)=>{
  $('#dataInputModal').modal('show');
  $.ajax({
    url: "forms/sales-form.php",
    type: "get",
    data:data,
    success: (data)=>{
 ...
 let submitFormSales = (e,args) =>{
  e.preventDefault();
  let oForm = document.getElementById('form-sales');
  ...
  $.ajax({
    url: "scripts/submit-sales-form.php",
    type: "post",
    dataType:"json",
    data:data,
    success: (data)=>{
...
**/
 let deleteSales = (salesId) =>{
  swal({
    ...
    $.ajax({
      url: "scripts/delete-sales-item.php",
      type: "post",
      dataType: "json",
...
$(document).ready(()=>{
  $('#link-view-sales').off().on('click',()=>{
    viewSales();
  });

  $('#add-new-sales').on('click',()=>{
    addNewSales();
  });
});
 ...
 return {
  ...
  viewSales:viewSales,
   submitFormSales : submitFormSales, 
