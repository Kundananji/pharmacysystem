...
?>
<form onsubmit = "Privilege.submitFormPrivilegePrivileges_admin(event,{<?php echo sizeof($arguments)>0?(implode(",",$arguments)):null ?>})" method="post" enctype="multipart/form-data" action="#" id="form-privilege">
...
<?php
  ...
  //override default value with actual value if object is sent
    if($privilegeEdit->getId()!=null){ $defaultValues['id']=$privilegeEdit->getId();};
?>
  <input type="hidden" name="id" id="input-privilege-id" value="<?php echo (isset($defaultValues['id'])?($defaultValues['id']): "0");?>"/>
<?php
...
<div class="form-group input-privilege-name">
  ...
  <input type="text" name="name" id="input-privilege-name" class="form-control " placeholder="Enter Name* " value="<?php echo (isset($defaultValues['name'])?($defaultValues['name']): "");?>" required <?php echo $readonly;?>   />
</div> <!--end form-group-->
<?php
    ...
    <select name="profileId" id="input-privilege-profile-id" class="form-control " required <?php echo $readonly;?> >
      ...
      <?php
        foreach($objects as $profile){
          ...
          $selected  =  isset($defaultValues['profileId']) && $defaultValues['profileId']==$optionValue || $privilegeEdit->getProfileId()==$profile->getId() ?"selected":"" ;
