...
$arguments=array();$paymentMethodId = isset($_GET['paymentMethodId'])?filter_var($_GET['paymentMethodId'], FILTER_VALIDATE_INT):null;
$paymentMethodsEdit = new PaymentMethods();
$paymentMethodsEditDao = new PaymentMethodsDao();
if(isset($paymentMethodId)){
  $tempObject = $paymentMethodsEditDao->select($paymentMethodId);
  if($tempObject !=null){
...
<div class="alert alert-info">Fields marked with an asterisk(*) are required.</div>
  ...
  <input type="hidden" name="paymentMethodId" id="input-payment-methods-payment-method-id" value="<?php echo null!==($paymentMethodsEdit->getPaymentMethodId())?($paymentMethodsEdit->getPaymentMethodId()):(isset($defaultValues['paymentMethodId'])?($defaultValues['paymentMethodId']): "0");?>"/>
...
<div class="form-group input-payment-methods-name">
                 ...
                 <?php
                  ...
                  if($paymentMethodsEdit->getPaymentMethodId()!=null){ $defaultValues['name']=$paymentMethodsEdit->getName();};
                  ?>
...
<div class="form-group input-payment-methods-description">
                 ...
                 <?php
                  ...
                  //override default value with actual value if object is sent
                  if($paymentMethodsEdit->getPaymentMethodId()!=null){ $defaultValues['description']=$paymentMethodsEdit->getDescription();};
                  ?>
...
<div class="form-group input-payment-methods-status">
                 ...
                 <?php
                  ...
                  //override default value with actual value if object is sent
                  if($paymentMethodsEdit->getPaymentMethodId()!=null){ $defaultValues['status']=$paymentMethodsEdit->getStatus();};
                  ?>
